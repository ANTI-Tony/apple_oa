        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .strokeBorder(theme.accent.color.opacity(isSelected ? 0.55 : 0), lineWidth: 1.5)
            )
            .padding(-8)
                ...
                Image(nsImage: image)
                    ...
                    .overlay(alignment: .bottomLeading) {
                        if !block.hasAcceptableAltText {
                            // The one place the canvas nags: an image nobody described.
                            Button {
                                actions.select()
                                actions.showInspector()
                            } label: {
                                Label("Add Description", systemImage: "exclamationmark.circle.fill")
                            }
